...
public class GoalGraph6 extends JFrame {
	...
	public GoalGraph6() {
		...
		goals.add(new Goal(400, 500, Goal.SUCCESS, new Date(System.currentTimeMillis())));
		...
		for (int i = 0; i < goals.size(); i++) {
			...
			goalInfoPanel.add(new JLabel(String.format("%.2f", goals.get(i).startDate)));
		...
		goalInfoScroll.setPreferredSize(new Dimension(400,100));
		//.add(goalInfoPanel);
	   ...
	   // fc.set
	    ...
	    Object[][] goalsCompletedData = new Goal[completedGoals][6];
	    ...
	    for (int i = 0; i < goals.size(); i++) {	    	
	    	if (goals.get(i).status == Goal.SUCCESS || goals.get(i).status == Goal.FAILURE) {
	    		...
	    		goalsCompletedData[--completedGoals][3] = goals.get(i).workLoad / goals.get(i).timeLimit;
	    		...
	    		goalsInProgressData[--goalsInProgress][0] = "TBD";
	    		...
	    		goalsInProgressData[--goalsInProgress][4] = goals.get(i).workLoad / goals.get(i).timeLimit;
	    		goalsInProgressData[--goalsInProgress][5] = goals.get(i).startDate;
	    		goalsInProgressData[--goalsInProgress][6] = goals.get(i).endDate;	   
	    	}
	    	else {
	    		goalsInProgressData[--goalsAwaitingInput][0] = "TBDawp";
	    		goalsInProgressData[--goalsAwaitingInput][1] = "TBD2awp";
	    		goalsInProgressData[--goalsAwaitingInput][2] = goals.get(i).workLoad;
	    		goalsInProgressData[--goalsAwaitingInput][3] = goals.get(i).timeLimit;
	    		goalsInProgressData[--goalsAwaitingInput][4] = goals.get(i).workLoad / goals.get(i).timeLimit;
	    		goalsInProgressData[--goalsAwaitingInput][5] = goals.get(i).startDate;
	    		goalsInProgressData[--goalsAwaitingInput][6] = goals.get(i).endDate;
	    	}
	    }
	    //TODO THIS
		...
		rightPanel.add(JTGoalsCompleted);
		rightPanel.add(JTGoalsAwaitingInput);
		rightPanel.add(JTGoalsInProgress);
		add(rightPanel);
	}
	...
	class ButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent e){
			if (e.getSource() == jbEnter) {
				...
				int returnVal = fc.showSaveDialog(GoalGraph6.this);
				...
				int returnVal = fc.showOpenDialog(GoalGraph6.this); // ???
				if (returnVal == JFileChooser.APPROVE_OPTION) {
	...
	public static void main(String[] args) {
		GoalGraph6 frame = new GoalGraph6();
